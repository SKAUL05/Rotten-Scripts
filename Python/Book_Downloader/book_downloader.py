...
if not os.path.exists(directory_name):
    ...
    if print_status:
        print(f"{directory_name} was successfully created.")
...
for url in loop[1:]:
    ...
    with open(f'{directory_name}/{book_name}', 'wb') as bin_file:  # Writing the file using open function
        bin_file.write(response.content)

    if print_status:
        print(f"{book_name} was downloaded.")
